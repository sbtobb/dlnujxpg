...
def login(username,password):
    ...
    if response.status_code != 200:
        ...
        return False
    response = session.get("http://authserver.dlnu.edu.cn/authserver/login?service=http%3A%2F%2Fzhjw.dlnu.edu.cn%2Flogin.jsp",headers=headers)
    if response.status_code != 200:
    ...
    response = session.post(url="http://authserver.dlnu.edu.cn/authserver/login?service=http%3A%2F%2Fzhjw.dlnu.edu.cn%2Flogin.jsp",data=data,headers=headers)
